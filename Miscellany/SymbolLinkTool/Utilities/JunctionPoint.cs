...
namespace SymbolLinkTool.Utilities {
	...
	public static class JunctionPoint {
		...
		///     Only works on NTFS.
		/// </remarks>
		/// <param name="srcDir">The junction point path</param>
		/// <param name="dstDir">The target directory</param>
		...
		/// </exception>
		public static void Create(string srcDir, string dstDir, bool overwrite) {
			...
			using var handle = OpenReparsePoint(srcDir, FileAccess.GenericWrite);
			byte[] targetDirBytes = Encoding.Unicode.GetBytes(NonInterpretedPathPrefix + Path.GetFullPath(dstDir));
		...
		public static void Delete(string junctionPoint) {
			...
			using var handle = OpenReparsePoint(junctionPoint, FileAccess.GenericWrite);
			var reparseDataBuffer = new ReparseDataBuffer {
				ReparseTag = IO_REPARSE_TAG_MOUNT_POINT,
				ReparseDataLength = 0,
				PathBuffer = new byte[0x3ff0]
		...
		public static bool Exists(string path) {
			...
			using var handle = OpenReparsePoint(path, FileAccess.GenericRead);
		...
		public static string GetTarget(string junctionPoint) {
			using var handle = OpenReparsePoint(junctionPoint, FileAccess.GenericRead);
			string target = InternalGetTarget(handle);
			if (target == null)
				throw new IOException("Path is not a junction point.");
